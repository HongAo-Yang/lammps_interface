...
try:
    ...
    print(
        "Warning: could not load networkx module, this is needed to produce the lammps data file."
    )
    sys.exit()
...
class MolecularGraph(nx.Graph):
    ...
    def reorder_labels(self, reorder_dic):
        ...
        for node, data in old_nodes:
            ...
            if "impropers" in data:
                ...
                for (a, c, d), val in imp_data:
                    ...
                    data["impropers"][
                        (reorder_dic[a], reorder_dic[c], reorder_dic[d])
                    ] = val
        ...
        for (a, b), val in old_edge_dict:
            self.sorted_edge_dict[(reorder_dic[a], reorder_dic[b])] = (
                reorder_dic[val[0]],
                reorder_dic[val[1]],
            )
    ...
    def add_atomic_node(self, **kwargs):
        ...
        if label not in kwargs:
            ...
            if label not in kwargs:
                print(
                    "ERROR: could not find the keyword for the element types in the cif file!"
        ...
        if set(orig_keys) & set(charge_keywords):
            ...
            try:
                ...
                print(
                    "Warning %s could not be converted " % (kwargs[key])
    ...
    def compute_bond_image_flag(self, n1, n2, cell):
        ...
        if dist > 7:
            ...
            print(
                "WARNING: bonded atoms %i and %i are %.3f Angstroms apart."
                % (n1, n2, dist)
    ...
    def coplanar(self, node):
        ...
        for v in vects[2:]:
            ...
            if not np.allclose(np.dot(v, n), 0.0, atol=0.02):
                return False
        return True
        ...
        for node, data in self.nodes_iter2(data=True):
            ...
            try:
                coordinates = np.array(
                    [float(del_parenth(data[i])) for i in coord_keys]
                )
            except KeyError:
                coordinates = np.array(
                    [float(del_parenth(data[i])) for i in fcoord_keys]
                )
    ...
    def compute_min_img_distances(self, cell):
        ...
        for n1, n2 in itertools.combinations(self.nodes(), 2):
            ...
            coords1, coords2 = (
                self.nodes[n1]["cartesian_coordinates"],
                self.nodes[n2]["cartesian_coordinates"],
            )
            ...
            if dist < 0.1:
                print(
                    "ERROR: distance between atom %i and %i are less than 0.1 Angstrom in the unit cell!"
            ...
            for n in list(self.neighbors(node)):
                ...
                try:
                    ...
                    if cycle_shortest_path <= cycle_shortest_path_cutoff:
                        cycle = list(
                            nx.all_simple_paths(
                                self, source=node, target=n, cutoff=cycle_shortest_path
                            )
                        )
    ...
    def compute_bond_typing(self):
            ...
            if set(elements) == set(["C", "O"]):
                    ...
                    if self.degree(at) == 1:
                        if self.degree(oxy) == 1:
                            ...
                            if set(oxynelem) <= organic:
                                ...
                                oxy_data["hybridization"] = "sp2"
                                data["order"] = (
                                    1  # this is the ether part of an ester...
                                )
                            # carboxylate?
                            else:
                        ...
                        if set(atnelem) <= organic:
                            ...
                            if len(oxynn) == 0:
                                car_data["hybridization"] = "sp2"
                                oxy_data["hybridization"] = "sp2"
                                data["order"] = 2.0  # carbonyl part of ester
                ...
                if (not "O" in carnelem) and (not "N" in carnelem):
                    if len(oxynn) > 0:
                        ...
                        if (
                            car_data["cycle"]
                            and car_data["hybridization"] == "aromatic"
                        ):
                            oxy_data["hybridization"] = "aromatic"
                            data["order"] = 1.5
                        # carbonyl
                ...
                if car_data["hybridization"] == "aromatic" and set(["H"]) == set(
                    nitnelem
                ):
                    data["order"] = 1.5
                ...
                (not self.nodes[n1]["cycle"])
                and (not self.nodes[n2]["cycle"])
                and (set(elements) <= organic)
            ):
        ...
        while double_check:
            ...
            for k in self.iterate_linear_chains(start, visited=[], excluded=[]):
                bond_orders = []
                ...
                for idx in range(len(k) - 1):
                    ...
                    if idx == 0:
                        ...
                        order = bond_orders[idx]
                        if (hyb1 == "sp2") and (hyb2 == "sp2"):
                            if (prev_order == 2.0) and (next_order == 1):
                                bond_orders[idx - 1] = 1.5
                                bond_orders[idx] = 1.5
                            elif (prev_order == 2.0) and (next_order == 2.0):
                                bond_orders[idx] = 1.0
                        elif (hyb1 == "sp") and (hyb2 == "sp"):
                            if (prev_order == 3.0) and (next_order == 3.0):
                                bond_orders[idx] = 1.0
                    else:
                        prev_order = bond_orders[idx - 1]
                        order = bond_orders[idx]
                        if (hyb1 == "sp2") and (hyb2 == "sp2"):
                            if (prev_order == 2.0) and (order == 2):
                                if set([elem1, elem2]) == set(["C", "O"]):
                                    onode = (
                                        n2 if self.nodes[n2]["element"] == "O" else n1
                                    )
    ...
    def iterate_linear_chains(self, start_node, visited=None, excluded=None):
        """
        迭代版本：查找两个金属(或终端原子)之间的所有独特链
        """
        # 初始化参数
        if visited is None:
            visited = []
        ...
        # 使用栈来模拟递归
        stack = [(start_node, visited[:], excluded[:])]

        while stack:
            node, current_visited, current_excluded = stack.pop()
            ...
            neighbors = [
                ...
                if i not in current_excluded and self.nodes[i]["element"] != "H"
            ]

            # 处理不同情况
            if not neighbors:  # 没有邻居
                if node in current_excluded and not current_visited:
                    continue
                elif node in current_excluded:
                    if current_visited:
                        current_visited.pop()
                else:
                    current_excluded.append(node)
                    current_visited.append(node)
                    yield current_visited
                    if current_visited:
                        current_visited.pop()
            else:  # 有邻居
                current_excluded.append(node)
                current_visited.append(node)
                # 将下一个节点添加到栈中
                stack.append((neighbors[0], current_visited[:], current_excluded[:]))

    def iterate_bonds_to_end(self, start_node):
        ...
        while queue:
            ...
            if self.nodes[node]["element"] in metals:
                continue

            neighbors = [
                i
                for i in self.neighbors(node)
                if i not in visited and self.nodes[i]["element"] != "H"
            ]
        ...
        for b, c, data in self.edges_iter2(data=True):
            ...
            for a in b_neighbours:
                for d in c_neighbours:
                    if not a == d:
                        data.setdefault("dihedrals", {}).update(
                            {(a, d): {"potential": None}}
                        )
    ...
    def compute_improper_dihedrals(self):
        ...
        for b, data in self.nodes_iter2(data=True):
            ...
            for idx in range(0, 6, 2):
                ...
                data.setdefault("impropers", {}).update(
                    {(a, c, d): {"potential": None}}
                )
    ...
    def compute_topology_information(self, cell, tol, num_neighbours):
        ...
        if self.find_metal_sbus:
            self.detect_clusters(
                num_neighbours, tol
            )  # num neighbors determines how many nodes from the metal element to cut out for comparison
    ...
    def sorted_node_list(self):
        return [
            n[1]
            for n in sorted(
                [(data["index"], node) for node, data in self.nodes_iter2(data=True)]
            )
        ]
    ...
    def img_offset(self, cells, cell, maxcell, flag, redefine, n1=0):
        ...
        if (np.any(newocell >= maxcell, axis=0)) or (np.any(newocell < 0.0)):

            # get indices of where the newocell is spanning a pbc
            indexes = (
                np.where(newocell - maxcell >= 0)[0].tolist()
                + np.where(newocell < 0)[0].tolist()
            )
    ...
    def correspondence_graph(self, graph, tol, general_metal=False, node_subset=None):
        ...
        for i, j in itertools.product(node_subset, graph_nodes):
            ...
            if general_metal:
                if (
                    ATOMIC_NUMBER.index(elementi) in METALS
    ...
    def detect_clusters(
        self, num_neighbors, tol, type="Inorganic", general_metal=False
    ):
        ...
        for node, data in self.nodes_iter2(data=True):
            if (
                (type == "Inorganic")
                and (general_metal)
                and (data["atomic_number"] in METALS)
                and ("special" not in data.keys())
            ):  # special means that this atom has already been found in a previous clique detection
        ...
        for node in reference_nodes:
            ...
            try:
                ...
                if data["element"] == "C":
                    chk_neighbors = (
                        num_neighbors  # organic clusters can be much bigger.
                    )
                else:
                ...
                for j in range(chk_neighbors):
                    ...
                    for n in instanced_neighbours:
                        if "special" not in self.nodes[n].keys():
                            ...
                            temp_neighbours += [
                                j for j in self.neighbors(n) if j not in neighbour_nodes
                            ]
                ...
                clusters = [
                    (cluster.number_of_nodes(), name, cluster)
                ...
                for count, name, cluster in list(reversed(sorted(clusters))):
                    ...
                    if (len(neighbour_nodes) + 1) < cluster.number_of_nodes():
                        continue
                    cg = self.correspondence_graph(
                        cluster,
                        toln,
                        general_metal=general_metal,
                        node_subset=neighbour_nodes + [node],
                    )
                    ...
                    for clique in cliques:
                        ...
                        if len(clique) == cluster.number_of_nodes():
                            ...
                            for i, j in clique:
                                self.nodes[i]["special_flag"] = cluster.nodes[j][
                                    "special_flag"
                                ]
                            cluster_found = True
                            print("Found %s" % (name))
                            store_sbus.setdefault(name, []).append(
                                [i for (i, j) in clique]
                            )
                            # break
        ...
        for j in set(no_cluster):
            print(
                "No recognizable %s clusters for %i elements %s"
                % (type.lower(), no_cluster.count(j), j)
            )
    ...
    def redefine_lattice(self, redefinition, lattice):
        ...
        if vol_change > 20:
            print(
                "ERROR: The volume change is %i times greater than the unit cell. "
                % (vol_change)
        ...
        print(
            "The redefined cell will be %i times larger than the original."
            % (int(vol_change))
        )
    ...
    def build_supercell(
        self, sc, lattice, track_molecule=False, molecule_len=0, redefine=None
    ):
        ...
        origincell = np.array([0.0, 0.0, 0.0])
        ...
        for count, cell in enumerate(cells):
            ...
            print(
                "Prog. = {}/{}; cell = {}x{}x{}".format(
                    count + 1, len(cells), cell[0][0], cell[1][0], cell[2][0]
                )
            )
            ...
            if count == 0:
                ...
                # rename nodes
                graph_image = nx.relabel_nodes(
                    deepcopy(orig_copy),
                    {
                        unit_node_ids[i - 1]: offset + unit_node_ids[i - 1]
            ...
            for i in range(1, totatomlen + 1):
                graph_image.nodes[unit_node_ids[i - 1] + offset]["image"] = (
                    unit_node_ids[i - 1]
                )
            if track_molecule:
            ...
            for node in graph_image.nodes():
                ...
                data["cartesian_coordinates"] = (
                    data["cartesian_coordinates"] + cartesian_offset
                )
                    ...
                    for (a, c), val in list(data["angles"].items()):
                        ...
                        if order_ba != (node, aid) and e_ba["symflag"] != ".":
                            ba_symflag = "1_%i%i%i" % (
                                tuple(
                                    np.array([10, 10, 10])
                                    - np.array([int(j) for j in e_ba["symflag"][2:]])
                                )
                            )
                        ...
                        if order_bc != (node, cid) and e_bc["symflag"] != ".":
                            bc_symflag = "1_%i%i%i" % (
                                tuple(
                                    np.array([10, 10, 10])
                                    - np.array([int(j) for j in e_bc["symflag"][2:]])
                                )
                            )
                        os_a = (
                            self.img_offset(
                                cells, newcell, maxcell, ba_symflag, redefine
                            )
                            * unitatomlen
                        )
                        os_c = (
                            self.img_offset(
                                cells, newcell, maxcell, bc_symflag, redefine
                            )
                            * unitatomlen
                        )
                        data["angles"].pop((a, c))
                ...
                try:
                    for (a, c, d), val in list(data["impropers"].items()):
                        ...
                        if order_ba != (node, aid) and e_ba["symflag"] != ".":
                            ba_symflag = "1_%i%i%i" % (
                                tuple([10 - int(j) for j in e_ba["symflag"][2:]])
                            )
                        if order_bc != (node, cid) and e_bc["symflag"] != ".":
                            bc_symflag = "1_%i%i%i" % (
                                tuple([10 - int(j) for j in e_bc["symflag"][2:]])
                            )
                        if order_bd != (node, did) and e_bd["symflag"] != ".":
                            bd_symflag = "1_%i%i%i" % (
                                tuple([10 - int(j) for j in e_bd["symflag"][2:]])
                            )
                        ...
                        os_d = (
                            self.img_offset(
                                cells, newcell, maxcell, bd_symflag, redefine
                            )
                            * unitatomlen
                        )
                        data["impropers"].pop((a, c, d))
            ...
            for v1, v2 in graph_image.edges():
                ...
                if data["symflag"] != ".":
                    ...
                    unit_repr = np.array([5, 5, 5], dtype=int)
                    translation = tuple(
                        np.array([int(j) for j in data["symflag"][2:]]) - unit_repr
                    )
                    # unique_translations.setdefault(translation,0)
                    # unique_translations[translation] += 1
                    # DEBUGGING
                    os_id = self.img_offset(
                        cells, newcell, maxcell, data["symflag"], redefine, n1
                    )
                    ...
                    #    print(redefine)
                    # pain...
                    opposite_flag = "1_%i%i%i" % (
                        tuple(
                            np.array([10, 10, 10])
                            - np.array([int(j) for j in data["symflag"][2:]])
                        )
                    )
                    rev_n1_img = (
                        self.img_offset(
                            cells, newcell, maxcell, opposite_flag, redefine
                        )
                        * unitatomlen
                        + n1_orig
                    )
                    ...
                    try:
                        for (a, d), val in list(data["dihedrals"].items()):
                            ...
                            offset_a = offset
                            if (
                                order_n1_a != (n1, a + offset)
                                and edge_n1_a["symflag"] != "."
                            ):
                                n1a_symflag = "1_%i%i%i" % (
                                    tuple(
                                        np.array([10, 10, 10])
                                        - np.array(
                                            [int(j) for j in edge_n1_a["symflag"][2:]]
                                        )
                                    )
                                )
                            if edge_n1_a["symflag"] != ".":
                                offset_a = (
                                    self.img_offset(
                                        cells, newcell, maxcell, n1a_symflag, redefine
                                    )
                                    * unitatomlen
                                )
                            ...
                            if (
                                order_n2_d != (n2, d + offset)
                                and edge_n2_d["symflag"] != "."
                            ):
                                n2d_symflag = "1_%i%i%i" % (
                                    tuple(
                                        np.array([10, 10, 10])
                                        - np.array(
                                            [int(j) for j in edge_n2_d["symflag"][2:]]
                                        )
                                    )
                                )
                    ...
                    data["symflag"] = self.update_symflag(
                        newcell, data["symflag"], origincell, maxcell
                    )
                    ...
                    try:
                        for (a, d), val in list(data["dihedrals"].items()):
                            ...
                            if edge_n2_d["symflag"] != ".":
                                offset_d = (
                                    self.img_offset(
                                        cells, newcell, maxcell, n2d_symflag, redefine
                                    )
                                    * unitatomlen
                                )
        ...
        while nodelist or queue:
            for n2, data in self[n1].items():
                if n2 not in queue and n2 in nodelist:
                    ...
                    dists = dists[0].tolist()
                    image = dists.index(min(dists))
                    self.nodes[n2]["cartesian_coordinates"] += np.dot(
                        supercells[image], cell.cell
                    )
                    data["symflag"] = "."
...
def write_CIF(graph, cell):
    ...
    c.add_data(
        "data", _audit_creation_method=CIF.label("Lammps Interface v.%s" % (str(0)))
    )
    ...
    for node, data in graph.nodes_iter2(data=True):
        ...
        c.add_data(
            "atoms", _atom_site_type_symbol=CIF.atom_site_type_symbol(data["element"])
        )
        try:
            c.add_data(
                "atoms",
                _atom_site_description=CIF.atom_site_description(
                    data["force_field_type"]
                ),
            )
        except KeyError:
            c.add_data(
                "atoms",
                _atom_site_description=CIF.atom_site_description(data["element"]),
            )
        ...
        try:
            c.add_data(
                "atoms",
                _atom_type_partial_charge=CIF.atom_type_partial_charge(data["charge"]),
            )
        except KeyError:
    ...
    tosort = [
        (data["order"], (n1, n2, data)) for n1, n2, data in graph.edges_iter2(data=True)
    ]
...
def write_PDB(graph, cell):
    ...
    # remark section
    pdbfile.writelines(
        "%6s %3i PDB file created by Lammps intervace v.%i\n" % ("REMARK", 1, 0)
    )
    pdbfile.writelines("%6s %3i Created on %s\n" % ("REMARK", 2, get_time()))
    # write crystallographic data
    pdbfile.writelines(
        "%6s%9.3f%9.3f%9.3f%7.2f%7.2f%7.2f %11s%4i\n"
    ...
    for node, d in graph.nodes(data=True):  # nx2.1
        ...
        pdbfile.writelines(
            "%-6s%5i %4s%1s%3s% 1s%3i%1s   %8.3f%8.3f%8.3f%6.2f%6.2f          %2s%2s\n"
            % (
                "ATOM",  # 1 - 6: ATOM
        ...
        num_lines = math.ceil(len(neighbs) / 4.0)
...
def write_RASPA_CIF(graph, cell, classifier=0):
    ...
    for node, data in sorted(graph.nodes_iter2(data=True)):
        ...
        if classifier == 0:
            c.add_data(
                "atoms", _atom_site_label=CIF.atom_site_label(data["force_field_type"])
            )
        elif classifier == 1:
            c.add_data("atoms", _atom_site_label=CIF.atom_site_label(data["ciflabel"]))
        ...
        c.add_data("atoms", _atom_site_fract_z=CIF.atom_site_fract_z(fc[2]))
        c.add_data(
            "atoms", _atom_site_charge=CIF.atom_type_partial_charge(data["charge"])
        )
    # bond block
    # must re-sort them based on bond type (Mat Sudio)
...
def write_RASPA_sim_files(lammps_sim, classifier=0):
    ...
    for i in range(len(data_list)):
        ...
        if data_list[i]["node"] < max_image:
            ...
            try:
                ...
                sig_ = str(UFF_DATA[fftype_][2] * (2 ** (-1.0 / 6.0)))
            except:
                print("%s %s not found!\n" % (element_, fftype_))
                continue

            # finally, add strings to list for each RASPA file
            MOF_PSEUDO_ATOMS.append(
                [
                    type_spec_,
                    print_,
                    as_,
    ...
    col_widths_final = [
        max(col_widths[i], col_widths1[i]) for i in range(len(col_widths))
    ]
    ...
    for i in range(len(MOF_PSEUDO_ATOMS)):
        ...
        for j in range(len(MOF_PSEUDO_ATOMS[0])):
            buff = "".join(
                " " for i in range(col_widths_final[j] - len(MOF_PSEUDO_ATOMS[i][j]))
            )
    ...
    for i in range(len(GENERIC_PSEUDO_ATOMS)):
        ...
        for j in range(len(GENERIC_PSEUDO_ATOMS[0])):
            buff = "".join(
                " "
                for i in range(col_widths_final[j] - len(GENERIC_PSEUDO_ATOMS[i][j]))
            )
    ...
    for i in range(len(MOF_FF_MIXING)):
        ...
        for j in range(len(MOF_FF_MIXING[0])):
            buff = "".join(
                " " for i in range(col_widths_final[j] - len(MOF_FF_MIXING[i][j]))
            )
    ...
    for i in range(len(GENERIC_FF_MIXING)):
        ...
        for j in range(len(GENERIC_FF_MIXING[0])):
            buff = "".join(
                " " for i in range(col_widths_final[j] - len(GENERIC_FF_MIXING[i][j]))
            )
...
class MDMC_config(object):
    ...
    def __init__(self, lammps_sim):
        ...
        try:
            ...
            print(
                "Warning! No MDMC.config file found.  LAMMPS sim files will not have guest molecule info"
            )
            return
        ...
        for line in lines:
            ...
            if parsed[0] == "pair_coeff":
                parsed[1] = str(
                    len(lammps_sim.unique_atom_types.keys()) + self.type_guest
                )
                for word in parsed:
                    outlines += word + " "
                outlines += "\n"
...
class Cell(object):
    ...
    def minimum_supercell(self, cutoff):
        ...
        widths = [
            np.dot(self.cell[0], b_cross_c) / np.linalg.norm(b_cross_c),
    ...
    def orthogonal_transformation(self):
        ...
        absmat[np.where(np.allclose(absmat, 0.0, atol=zero_tol))] = 0.0
    ...
    def __mkparam(self):
        ...
        alpha = (
            np.arccos(sum(self.cell[1, :] * self.cell[2, :]) / (cell_b * cell_c))
            * 180
            / np.pi
        )
        beta = (
            np.arccos(sum(self.cell[0, :] * self.cell[2, :]) / (cell_a * cell_c))
            * 180
            / np.pi
        )
        gamma = (
            np.arccos(sum(self.cell[0, :] * self.cell[1, :]) / (cell_a * cell_b))
            * 180
            / np.pi
        )
